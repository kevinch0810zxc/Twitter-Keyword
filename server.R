...
shinyServer(function(input, output) {
  ...
  observeEvent(input$search, handlerExpr = {
    ...
    sent.unlist <- unlist(temp)
    output$babbleTable <- renderPrint({
      validate(
        need(try(input$ngram <= input$genlength), "Selected N-Gram value greater than requested tweet length")
      )
      ng <- ngram(sent.unlist, input$ngram)
      babble(ng, input$genlength)
    })
  ...
  observeEvent(input$search2, handlerExpr = {
    ...
    sent.unlist <- unlist(temp)
    output$babbleTable2 <- renderPrint({
      validate(
        need(try(input$ngram2 <= input$genlength2), "Selected N-Gram value greater than requested tweet length")
      )
      ng <- ngram(sent.unlist, input$ngram2)
      babble(ng, input$genlength2)
    })
